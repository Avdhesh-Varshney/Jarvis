...
def API_exists():
    ...
    try:
        load_dotenv(dotenv_path="source/features/chatBots/secrets/.env")
...
def instructions_to_set_API():
    ...
    if not API_exists():
        ...
        st.markdown(
            "Create an empty file called ```.env``` in the secrets folder found as ```source/features/chatBots/secrets/```."
        )
        st.markdown(
        ...
        st.markdown(
            "The chatbot will be usable until you delete that API key from the file or until you have reached the rate limits of the API key ."
        )
