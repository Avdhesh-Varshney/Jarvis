...
def add_to_expression(symbol, display_symbol=None):
    # Automatically insert '*' if needed
...
def calculate_expression():
    try:
        # Safe evaluation using math module functions and custom trigonometric functions
...
# Map functions and operators to their corresponding symbols
func_mappings = {
    'sin': ('sin(', 'sin('),
    'cos': ('cos(', 'cos('),
    'tan': ('tan(', 'tan('),
...
# Display angle mode and toggle button
    ...
    for i, label in enumerate(row):
        if label:
            ...
            if label == 'AC':
                ...
                cols[i].button(label, on_click=toggle_angle_mode, key=key)
            else:
                # Get the symbol and display symbol from func_mappings
                symbol, display_symbol = func_mappings.get(label.strip('`'), (label.strip('`'), label.strip('`')))
                # Adjust the label to prevent Markdown parsing issues
                if label in ('`+`', '`-`'):
                    button_label = label  # Already wrapped in backticks
                else:
                    button_label = label
                cols[i].button(button_label, on_click=add_to_expression, args=(symbol, display_symbol), key=key)
